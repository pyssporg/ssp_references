    ...
    with SSP(ssp_path, mode="w") as ssp:
        ...
        with ssp.system_structure() as ssd:
            ...
            system.connections.extend(
                [
                    Connection(start_element="sine", start_connector="y", end_element="add", end_connector="u1"),
                    Connection(start_element="gain", start_connector="y", end_element="add", end_connector="u2"),
                    Connection(start_element="add", start_connector="y", end_element="gain", end_connector="u"),
                ]
            )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="signal_algebraic_loop_") as temp_dir:
