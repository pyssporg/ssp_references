...
def create_ssp(model: ModelMetaData, temp_dir: Path, exp: LSRefExperiment) -> None:
    step_a_path = temp_dir / "StepA.fmu"
    step_b_path = temp_dir / "StepB.fmu"
    ...
    with FMU(step_a_path, mode="a") as fmu:
        with fmu.model_description as md:
            md.strip_model_exchange()
    with FMU(step_b_path, mode="a") as fmu:
        with fmu.model_description as md:
            md.strip_model_exchange()
    ...
    with SSP(ssp_path, mode="w") as ssp:
        ...
        for parameters in exp.parameters:
            ssp.add_external_parameterset(
                MODEL_DIR / "ssp" / parameters.source,
                MODEL_DIR / "ssp" / parameters.mapping,
            )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="signal_step_add_") as temp_dir:
